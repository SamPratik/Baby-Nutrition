...
<?php
	if(isset($_POST["questionDisease"])) {
		$pat_name = $_SESSION["username"];
		$question = $_POST["question"];
		$question = mysqli_real_escape_string($connection,$question);
		
		$insertDisease = "INSERT INTO disease (pat_name,question) ";
		$insertDisease .= "VALUES ('{$pat_name}' , '{$question}')";	
...
<?php if($_SESSION["disease"] == "disease") { ?>
	...
	<form class="question" style="float:left;margin-top: 43px;margin-left: 55px;" action="patientForum.php" method="post">
		...
		<br/><br/><br/><br/>
		<input class="btn btn-primary btn-md" style="float:left; margin-left:625px;" type="submit" name="questionDisease" value="প্রশ্ন">
	</form>
...
<div class="comment-holder" style="float:left;margin-top: 20px;margin-left: -27px;">
	<?php
		$questionSelect = "SELECT * FROM disease ORDER BY id DESC";
	...
	?>
			<div class="comments" style="float:left; width:750px; border:2px solid black; margin-bottom:8px;margin-left: 85px;">
                ...
                <?php if(!$row["answer"] == NULL) { ?>
				    ...
				    <span style="float:left; width:745px"><strong style="color:green; margin-right:5px;">উত্তর: </strong><?php echo $row["answer"]; ?></span>
				<?php } ?>
